...
export default function DecryptGate() {
  ...
  const handleDecrypt = async () => {
    ...
    try {
      ...
      const [saltB64, ivB64, cipherB64] = hash.split(':');
    ...
    <div className="min-h-screen bg-slate-900 flex flex-col items-center justify-center p-6 text-white">
      <div className="w-full max-w-xl bg-slate-800 rounded-2xl shadow-2xl border border-slate-700 overflow-hidden">
        <div className="flex border-b border-slate-700">
          <button onClick={() => setMode('encrypt')} className={`flex-1 py-4 font-semibold flex items-center justify-center gap-2 ${mode === 'encrypt' ? 'bg-slate-700 text-indigo-400' : 'text-slate-400'}`}>
            <Lock className="w-4 h-4" /> Encrypt
          </button>
          <button onClick={() => setMode('decrypt')} className={`flex-1 py-4 font-semibold flex items-center justify-center gap-2 ${mode === 'decrypt' ? 'bg-slate-700 text-indigo-400' : 'text-slate-400'}`}>
          ...
          {mode === 'encrypt' ? (
            <div className="space-y-6">
              <div>
                ...
                <textarea value={inputText} onChange={e => setInputText(e.target.value)} className="w-full h-32 p-3 bg-slate-900 text-slate-200 outline-none resize-none text-sm rounded-lg border border-slate-700 focus:ring-2 focus:ring-indigo-500" placeholder="Enter your secret text or file data..." />
              </div>
              <div>
              ...
              <button onClick={handleEncrypt} className="w-full py-3 bg-indigo-600 hover:bg-indigo-500 rounded-lg font-semibold flex items-center justify-center gap-2">
                <Shield className="w-5 h-5" /> Generate Secure Link
              </button>

              {generatedLink && (
                <div className="bg-slate-900 p-4 rounded-lg border border-slate-700">
                  <p className="text-xs text-slate-400 mb-2">Share this link (URL contains encrypted data):</p>
                  <div className="flex items-center gap-2">
                    ...
                    <button onClick={copyLink} className="p-2 bg-slate-700 hover:bg-slate-600 rounded-md">
            ...
            <div className="space-y-6">
              <div className="text-center text-slate-400 text-sm">
                {window.location.hash ? 'Encrypted payload detected in URL.' : 'No encrypted payload found. Open a secure link to decrypt.'}
              </div>
              <div>
              ...
              <button onClick={handleDecrypt} className="w-full py-3 bg-indigo-600 hover:bg-indigo-500 rounded-lg font-semibold flex items-center justify-center gap-2">
